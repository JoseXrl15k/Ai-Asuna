...
let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  let txt = '`乂  W H A T M U S I C  -  T O O L S`\n\n'
      txt += `	♢˚₊· ͟͟͞͞🌹   *Titulo* : ${title}${artists ? `\n	♢˚₊· ͟͟͞͞🌹   *Artists* : ${artists.map(v => v.name).join(', ')}` : ''}`
      txt += `${album ? `\n	♢˚₊· ͟͟͞͞🌹   *Album* : ${album.name}` : ''}${genres ? `\n	♢˚₊· ͟͟͞͞🌹   *Genero* : ${genres.map(v => v.name).join(', ')}` : ''}\n`
      txt += `	♢˚₊· ͟͟͞͞🌹   *Fecha de lanzamiento* : ${release_date}\n\n`
      txt += `> 🚩 *${textbot}*`
