...
await star.sendMessage(m.chat, { document: { url: dl_url }, caption: '*By: AsunaBot*', mimetype: 'audio/mpeg', fileName: `${title}.mp3`}, { quoted: fkontak })
await m.react('✅')
} catch {
await m.react('🕒')
try {
...
if (size.split('MB')[0] >= limit) return star.reply(m.chat, `El archivo pesa mas de ${limit} MB, se cancelÃ³ la Descarga.`, m, rcanal).then(_ => m.react('✅'))
await star.sendMessage(m.chat, { document: { url: dl_url }, caption: '*By: Asuna Yuuki*', mimetype: 'audio/mpeg', fileName: `${title}.mp3`}, { quoted: fkontak })
await m.react('✅')
} catch {
await m.react('✖️')
}}}}
