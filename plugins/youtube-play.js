...
let handler = async (m, { conn: star, command, args, text, usedPrefix }) => {
  if (!text) return star.reply(m.chat, '🚩 Ingresa el título de un video o canción de YouTube.\n\n`Ejemplo:`\n' + `> *${usedPrefix + command}* Gemini Aaliyah - If Only`, m, rcanal)
    ...
    let txt = '`乂  Y O U T U B E  -  P L A Y`\n\n'
       txt += `	♢˚₊· ͟͟͞͞🌹   *Título* : ${res[0].title}\n`
       txt += `	♢˚₊· ͟͟͞͞🌹   *Duración* : ${secondString(res[0].duration.seconds)}\n`
       txt += `	♢˚₊· ͟͟͞͞🌹   *Publicado* : ${eYear(res[0].ago)}\n`
       txt += `	♢˚₊· ͟͟͞͞🌹   *Canal* : ${res[0].author.name || 'Desconocido'}\n`
       txt += `	♢˚₊· ͟͟͞͞🌹   *Url* : ${'https://youtu.be/' + res[0].videoId}\n\n`
