...
await conn.sendButton(m.chat, `\`ැ⏍⏎𝗔𝗦𝗨𝗡𝗔-𝗬𝗨𝗨𝗞𝗜⏍⏎ු\`

*» 𝗢𝗣𝗖𝗜𝗢𝗡 |* ${type.toUpperCase()}
*» 𝗘𝗦𝗧𝗔𝗗𝗢 |* ${isEnable ? 'ON' : 'OFF'}
*» 𝗣𝗔𝗥𝗔 |*  ${isAll ? 'ESTE BOT' : isUser ? '' : 'ESTE CHAT'}`, wm, null, [[`${isEnable ? `ᴅᴇꜱᴀᴄᴛɪᴠᴀʀ 🐈` : `ᴀᴄᴛɪᴠᴀʀ 🐢`}`, `${isEnable ? `.off ${type}` : `.on ${type}`}`]], null, null, estilo)
